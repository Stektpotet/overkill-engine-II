...
AnimatedSpriteRenderer::AnimatedSpriteRenderer(
    ...
    ) : SpriteRenderer<TextureAtlas>(gameObject, id, texture)
    {}
    ...
    void AnimatedSpriteRenderer::update(float deltaTime)
    {
        m_rotation += deltaTime * 0.25f;
        m_frame = int(m_rotation*90)%(m_texture.getDepth()-1);
    ...
    void AnimatedSpriteRenderer::draw()
    {
        ...
        m_VAO.bind();
        GFX_DEBUG("atlasIndex: %d", m_frame);
        // Set "update every frame" uniforms:
        GFX_GL_CALL(glUniform1i(m_shader.getUniformLocation("atlasIndex"), m_frame));
